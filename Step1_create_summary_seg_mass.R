...
summary$ID_time <- difftime(summary$datetime_min, summary$trainingstart, units = "hours")
...
#----------calculate respiration per kg/h from estimate_s and remove background respiration--------------#

#convert ID_time to numeric and store the "raw" absolute respiration
...
#substract background respiration from absolute respiration measured
...
resp <- summary[(summary$resp_absol <= 0),]
...
unreal <- summary[(summary$resp_absol/mean(summary$resp_absol) > 50),]
...
wrong_order <- summary %>% 
  filter(ID_temp_press == "733250_12_8" | ID_temp_press == "733250_12_1") %>% 
  mutate(temporary = min(datetime_min[which(ID_temp_press == "733250_12_1")])) %>% 
  filter(ID_temp_press == "733250_12_8" & datetime_min < temporary) %>% 
  mutate(temporary = NULL)
if (nrow(wrong_order) > 0) {wrong_order$condition <- "treatment at wrong time or time wrong (e.g. 8bar before 1bar)"}

#create a list of the dataframes created above
conditions <- list(tacclimatization, pacclimatization, r_squared, unreal, n, distance, duration, temp, press, press_s, resp, sat, p_lm, p_i, p_s, wrong_order)
...
removals_reason <- removals_reason %>%
  rename(removed_tacclimatization = "temperature acclimatization",
         ...
         removed_order = "treatment at wrong time or time wrong (e.g. 8bar before 1bar)"
  )
...
############################################ORDER DATA FRAME, CALCULATE RESP PER KG AND  AND EXPORT#################################################

#calculate mass at each point in time
resp_raw_summary <- summary_raw1%>% 
  group_by(ID) %>% 
  summarize(realstart = min(datetime_min),
            realend = max(datetime_max))

#summarize by ID and treatment and extract start/endtimes, -masses and mean respiration 
resp_summary <- summary %>%
  ungroup() %>% 
  group_by(ID, ID_temp_press) %>% 
  summarize(starttime = min(datetime_min),
            endtime = max(datetime_max),
            r_mean = mean(resp_absol),
            startmass = max(startmass),
            endmass = max(endmass),
            delta = startmass - endmass) %>% 
  left_join(resp_raw_summary, by = "ID") %>%
  ungroup() 

#edit start/endtimes of treatment period to account for acclimation, calculate total oxygen consumption per treatment period 
resp_summary <- resp_summary %>% 
  arrange(ID, starttime) %>%
  group_by(ID) %>% 
  mutate(init_start = starttime,
         init_end = endtime,
         starttime = if_else(str_detect(ID_temp_press, "_19_1"), realstart, starttime),
         endtime = if_else(str_detect(ID_temp_press, "_12_8"), realend, endtime),
         nextstart = lead(starttime, 1),
         testdiff = difftime(nextstart, endtime),
         trueend = as.POSIXct((as.numeric(endtime) + as.numeric(nextstart)) / 2, origin = '1970-01-01', tz = "UTC"),
         endtime = if_else(str_detect(ID_temp_press, "_12_8"), endtime, trueend),
         priorend = lag(endtime, 1),
         starttime = if_else(str_detect(ID_temp_press, "_19_1"), starttime, priorend), 
         r_time = as.numeric(difftime(endtime, starttime, units = "hours")),
         total_ox = r_time*r_mean) %>% 
  ungroup()

#calculate weight loss per period etc.
resp_summary <- resp_summary  %>% 
  group_by(ID) %>% 
  mutate(sum_ox = sum(total_ox),
         factor = total_ox/sum_ox,
         test_factor = sum(factor),
         delta_treat = factor*delta,
         test_delta = sum(delta_treat),
         loss_h = delta_treat/r_time,
         t_endmass = startmass - cumsum(delta_treat),
         t_startmass = ifelse(str_detect(ID_temp_press, "_19_1"), startmass, lag(t_endmass, 1))) %>% 
  ungroup()

back <- summary
summary <- back

#add relevant columns to summary and calculate mass at each point in time
summary <- summary %>%
  ungroup() %>% 
   left_join(resp_summary %>% select(ID_temp_press, starttime, t_startmass, t_endmass, loss_h), by = "ID_temp_press") %>% 
   mutate(losstime = as.numeric(difftime(datetime_min, starttime, units = "hours")),
          mass_now = t_startmass - losstime*loss_h,
          resp = resp_absol/(mass_now/1000)) %>% 
  relocate(resp, .before = resp_raw) %>% 
  relocate(resp_absol, .before = resp_raw) %>% 
  relocate(hr_calc, .before = resp_raw) %>% 
  relocate(mass_now, .before = resp_absol)

#graph to check result
massplot <- ggplot(summary, aes(x=datetime_min, y=mass_now)) +
