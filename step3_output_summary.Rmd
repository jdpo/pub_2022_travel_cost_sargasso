...
flextable(result %>%
  ...
  mutate(Parameter = if_else(Parameter == "(Intercept)", "Intercept", 
                             ...
                             if_else(Parameter == "speedBL", "Velocity (BL/s)", 
                             if_else(Parameter == "temp_mean:press_mean", "Temp/Press Interaction", "NA"))))))) %>%
  autofit() 
...
fig2c <-  tag_facet2(
  ggplot(summary, aes(x=temp_mean, y=resp)) +
  theme_bw()+
  geom_point(aes(x= temp_mean, y = resp, color = speedBL, shape = as.factor(npress)), alpha = 0.5)+
